// src/pages/Packages/KarnatakaPackages.jsx
import { useState, useEffect } from "react";
import { Link } from "react-router-dom";
import { motion, AnimatePresence } from "framer-motion";
import { filterOptions } from "./karnatakaPackageData";
import { getPackages } from "../../services/packageService";
import "./KarnatakaPackages.css";

// Import all destination images dynamically
const imageModules = import.meta.glob("../state/karnataka/**/*.{png,jpg,jpeg}", { eager: true });
...
const destinationImages = Object.entries(imageModules)
  ...
  .reduce((map, [path, moduleValue]) => {
    ...
    const match = normalized.match(/karnataka\/([^/]+)\/[^/]+\.(png|jpe?g)$/i);
    if (!match) return map;
...
const KarnatakaPackages = () => {
  const [karnatakaPackages, setKarnatakaPackages] = useState([]);
  ...
  useEffect(() => {
    const loadPackages = async () => {
      setLoading(true);
      const packages = await getPackages({ state: "Karnataka" });
      setKarnatakaPackages(packages);
      setLoading(false);
    };
    loadPackages();
  }, []);
  ...
  // Filter packages
  const filteredPackages = karnatakaPackages.filter((pkg) => {
  ...
  return (
    <div className="karnataka-packages-page">
      {/* Hero Section */}
      <section className="kap-hero">
        ...
        <div className="hero-content">
          ...
          >
            Explore handpicked experiences across Karnataka
          </motion.p>
        </div>
      </section>

      {/* Filters Section */}
      <section className="kap-filters-section">
        <div className="kap-container">
          ...
          <div className="results-count">
            ...
            <strong>{karnatakaPackages.length}</strong> packages
          </div>
        </div>
      </section>

      {/* Packages Section */}
      <section className="kap-packages-section">
        <div className="kap-container">
          {loading ? (
            ...
            <AnimatePresence mode="wait">
              {filteredPackages.length > 0 ? (
                ...
                >
                  ...
                  <p>Try adjusting your filters to find your perfect Karnataka escape.</p>
                </motion.div>
              )}
            </AnimatePresence>
          )}
    ...
    >
      ...
      <div className="card-info-section">
        ...
        <div className="card-footer">
          ...
          <Link to={`/karnataka-package/${pkg.id}`} className="select-btn">
            View Details
          </Link>
        </div>
      </div>
